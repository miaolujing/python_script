...
class Crondns(unittest.TestCase):
    ...
    def test_crondns(self):
        ...
        #新增定时任务
        if addtask.adddns('{ "url":"'+config.cronhttp+'","host":"'+config.httphost+'","referer":"","cookie":"","userAgent":"","maxSpeed":104857,"timeout":20,"method":"GET"}',config.cronhost2,"300","http") == False:
            print "post httpcrontask fail"
            driver.quit()
            os._exit(0)
        else:
            print "post httpcrontask suss"
        time.sleep(5)
        #判断定时任务新增是否成功
        if len(db.readtaskid(config.cronhttp)) < 1:
            print "add httpcrontask fail"
            driver.quit()
            os._exit(0)
        else:
            print "add httpcrontask suss"
        #新增监控图
        ...
        for i in range(0, len(datas)):
            if i < len(datas)-1:
                data += "metric="+config.httphost+" metric="+str(datas[i])+"\n"
            if i == len(datas)-1:
                data += "metric="+config.httphost+" metric="+str(datas[i])+""
            if i < len(datas)-1:
                counts +="metric="+config.httphost+" metric="+str(datas[i])+"|"
            if i == len(datas)-1:
                counts +="metric="+config.httphost+" metric="+str(datas[i])+""
        if addtask.addmonitor(config.httpname,config.dashhost2,"3600",data,"h","true") ==False:
            print "post httpmonitortask fail"
            driver.quit()
            os._exit(0)
        else:
            print "post httpmonitortask suss"
        time.sleep(5)
        #判断监控图任务是否新增成功
        if db.readmonitor(counts) == config.httpname:
            print "add httpmonitortask suss"
        else:
            print "add httpmonitortask fail"
            driver.quit()
            os._exit(0)
        time.sleep(300)
        #查看报表展现
        ...
        assertresult.graphgethttp()
        time.sleep(2)
        # 删除监控图管理
        driver.switch_to_window(driver.window_handles[2])
        time.sleep(2)
        driver.find_element_by_id("g_name").clear()
        driver.find_element_by_id("g_name").send_keys(config.httpname)
        ...
        #删除定时任务
        deletetask.crondel(config.cronhttp)
        time.sleep(2)
        #退出登录
        driver.switch_to_window(driver.window_handles[0])
        time.sleep(2)
